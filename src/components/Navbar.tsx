...
export default function Navbar({ activeTab, setActiveTab, darkMode, setDarkMode }: NavbarProps) {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md border-b border-emerald-100/60 dark:border-blue-900/30 transition-all duration-300 shadow-sm">
        ...
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-1 lg:space-x-1.5">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-85 bg-white/98 dark:bg-slate-900/98 backdrop-blur-xl rounded-2xl border border-emerald-100/80 dark:border-blue-900/40 shadow-2xl p-2 z-50 transition-all duration-200 animate-in fade-in slide-in-from-top-3">
                  <div className="px-3 py-2 text-[10px] font-extrabold uppercase tracking-widest bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent border-b border-slate-100 dark:border-slate-850 mb-1">
                    Fitur & Amalan Utama
                  </div>
                        ...
                        >
                          <div className={`p-2 rounded-lg flex items-center justify-center shrink-0 ${
                            isActive 
                              ...
                              : 'bg-emerald-50/70 dark:bg-blue-950/30 text-emerald-650 dark:text-blue-400 group-hover:bg-emerald-100 dark:group-hover:bg-blue-900/50'
                          }`}>
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <span className="block text-[10px] text-slate-400 dark:text-emerald-400/60 font-medium truncate mt-0.5">
                              {item.desc}
                            </span>
                          </div>
                          {isActive && (
                            <div className="w-2 h-2 rounded-full bg-blue-550 dark:bg-amber-350 animate-pulse mr-1" />
                          )}
                        </button>
                      );
                    })}
      ...
      {isOpen && (
        <div className="md:hidden border-t border-emerald-100/80 dark:border-blue-900/30 bg-white/98 dark:bg-slate-950/98 backdrop-blur-md transition-colors animate-in slide-in-from-top-4 duration-200">
          <div className="px-3 pt-3 pb-5 space-y-4">
            ...
            <div>
              <div className="px-3 pb-1 text-[9px] font-extrabold uppercase tracking-widest text-slate-400 border-t border-slate-100 dark:border-blue-900/20 pt-3">
                Kajian & Ibadah
              </div>
              <div className="space-y-1 mt-1">
                {dropdownItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className="w-4.5 h-4.5 mr-3 shrink-0 text-emerald-600 dark:text-blue-400" />
                      <div className="text-left w-full">
                        <span className="block">{item.label}</span>
                        <span className={`block text-[10px] -mt-0.5 ${isActive ? 'text-white' : 'text-slate-400 dark:text-emerald-400/50'}`}>{item.desc}</span>
                      </div>
                    </button>
                  );
                })}
